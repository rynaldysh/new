...
class InputJualBarangActivity : AppCompatActivity() {

    lateinit var myDb: MyDatabase

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_input_jual_barang)
        myDb = MyDatabase.getInstance(this)!!
        Helper().setToolbar(this, toolbarBiasa, "From Jual Barang")
        ...
        ApiConfig.instanceRetrofit.pesanjasa(pesanJasa).enqueue(object :Callback<ResponModel> {
            override fun onResponse(call: Call<ResponModel>, response: Response<ResponModel>) {
                ...
                if (respon.success == 1){
                    ...
                    val intent = Intent(this@InputJualBarangActivity, SuccesJasaActivity::class.java)
                    intent.putExtra("bokingjasa", jsPesanJasa)

                    startActivity(intent)
                    Toast.makeText(this@InputJualBarangActivity, "Berhasil memesan jasa ", Toast.LENGTH_SHORT).show()

                } else {
                    error(respon.message)
                    Toast.makeText(this@InputJualBarangActivity, "" + respon.message, Toast.LENGTH_SHORT).show()
                }
            }
